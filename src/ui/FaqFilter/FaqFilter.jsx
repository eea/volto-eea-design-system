  ...
  return (
    <div className={`menu row ${context.active ? 'active' : ''}`}>
      <div className="wrapper">
        ...
        <div
          className={`arrow ${context.active ? 'up' : ''} `}
        ...
        <div
          className={`clear ${context.selected.length > 0 ? '' : 'hidden'}`}
          onClick={() => context.clearFilters()}
          onKeyDown={() => context.clearFilters()}
          role="button"
          tabIndex={0}
        >
          {rest.clearText}
      ...
      <div className={`results ${context.selected.length > 0 ? '' : 'hidden'}`}>
        {rest.resultCount}
      </div>
    </div>
  );
        ...
        context.selected.map((active, index) => (
          <Label key={active}>
            ...
            <Icon
              ...
              onClick={() => context.removeFilter(active)}
            />
          </Label>
        ))}
    </div>
...
const Filters = ({ children, ...rest }) => {
  ...
  return (
    <div className={`filters ${context.active ? '' : 'hidden'}`}>
      <Grid>
        {rest.filters.map((filter) => (
          <Grid.Column mobile={12} tablet={6} computer={4} key={filter.id}>
            <Checkbox
