...
function ContentAccordionContainer({ ...args }) {
    ...
    return (
        <ContentAccordion className={args.readMoreVariant ? 'read more' : ''}>
            {args.content.length > 0 && args.content.map((item, index) => (
                <div key={index}>
                    ...
                    <ContentAccordion.Content active={activeIndex === index}>
                        <ContentAccordion.Description>{item.description}</ContentAccordion.Description>
...
const Template = (args) => <ContentAccordionContainer {...args}></ContentAccordionContainer>;
